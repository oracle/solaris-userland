...
function _isInvalidTarget(target) {
  ...
  const INVALID_TARGET_CLASSES = [// CM5 tokens,
  "cm-atom", "cm-number", "cm-operator", "cm-string", "cm-tag", // CM6 tokens,
  "tok-string", "tok-punctuation", "tok-number", "tok-operator", // also exclude editor element (defined in Editor component)
  "editor-mount"];
...
function _dispatch(codeMirrorOrSourceEditor, eventName, data) {
  if (_prefs.features.codemirrorNext) {
    ...
    codeMirrorOrSourceEditor.constructor.signal(codeMirrorOrSourceEditor, eventName, data);
  }
}
...
function onMouseOver(codeMirrorOrEditor) {
