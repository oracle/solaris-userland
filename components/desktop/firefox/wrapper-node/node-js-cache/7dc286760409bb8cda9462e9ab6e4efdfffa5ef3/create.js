  ...
  if (!(0, _selectors.hasSourceActor)(store.getState(), sourceActorId)) {
    await new Promise(resolve => {
      ...
      function check() {
        ...
        currentSize = store.getState().sourceActors.size; // For perf reason, avoid any extra computation if sources did not change
...
function makeSourceId(sourceResource) {
  ...
  if (sourceResource.url) {
    ...
    const threadActorID = sourceResource.targetFront.getCachedFront("thread").actorID;
...
function createSourceMapOriginalSource(id, url, thread) {
  return createSourceObject({
    id,
    url,
    thread,
...
function createPrettyPrintOriginalSource(id, url, thread) {
  return createSourceObject({
    id,
    url,
    thread,
...
function createSourceActor(sourceResource) {
  const actorId = sourceResource.actor; // As sourceResource is only SourceActor's form and not the SourceFront,
