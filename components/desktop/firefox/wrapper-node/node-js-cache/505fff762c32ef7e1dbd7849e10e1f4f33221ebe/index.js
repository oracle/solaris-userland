...
async function buildMappedScopes(source, content, frame, scopes, {
  ...
  const originalRanges = await (0, _rangeMetadata.loadRangeMetadata)(frame.location, originalAstScopes, sourceMaps);
    ...
    for (const name of Object.keys(item.bindings)) {
      for (const ref of item.bindings[name].refs) {
        ...
        for (const loc of locs) {
          precalculatedRanges.set(buildLocationKey(loc.start), sourceMaps.getGeneratedRanges(loc.start));
          precalculatedLocations.set(buildLocationKey(loc.start), sourceMaps.getGeneratedLocation(loc.start));
          precalculatedLocations.set(buildLocationKey(loc.end), sourceMaps.getGeneratedLocation(loc.end));
        }
      }
    }
  }
