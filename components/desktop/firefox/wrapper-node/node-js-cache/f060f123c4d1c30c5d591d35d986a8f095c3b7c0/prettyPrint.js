...
async function prettyPrintSource(source, thunkArgs) {
  const {
    dispatch,
    getState
  } = thunkArgs;
  ...
  await dispatch((0, _index3.mapFrames)(sourceActor.thread)); // Update breakpoints locations to the new pretty/original source
...
const memoizedPrettyPrintSource = (0, _memoizableAction.memoizeableAction)("setSymbols", {
  getValue: (source, {
    getState
  }) => {
...
function prettyPrintAndSelectSource(source) {
  return async ({
    ...
    const prettySource = await dispatch(memoizedPrettyPrintSource(source)); // Select the pretty/original source based on the location we may
