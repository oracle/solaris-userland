...
loader.lazyRequireGetter(this, "_index", "devtools/client/debugger/src/actions/ast/index");
...
const setSelectedLocation = (location, shouldSelectOriginalLocation, shouldHighlightSelectedLocation) => ({
...
function selectSourceURL(url, options) {
  return async ({
    ...
    const source = (0, _index2.getSourceByURL)(getState(), url);
...
async function mayBeSelectMappedSource(location, keepContext, thunkArgs) {
  ...
  let shouldSelectOriginalLocation = (0, _index2.getShouldSelectOriginalLocation)(getState());
  ...
  if (keepContext) {
    ...
    if (!location.source.isOriginal && shouldSelectOriginalLocation && (0, _index2.hasPrettyTab)(getState(), location.source)) {
    ...
    if (shouldSelectOriginalLocation != location.source.isOriginal) {
      ...
      if (location.source.isOriginal || (0, _index2.isSourceActorWithSourceMap)(getState(), location.sourceActor.id)) {
  ...
  } else if (location.source.isOriginal || (0, _index2.isSourceActorWithSourceMap)(getState(), location.sourceActor.id)) {
...
function selectLocation(location, {
  ...
  return async thunkArgs => {
    ...
    if (!sourceActor) {
      sourceActor = (0, _index2.getFirstSourceActorForGeneratedSource)(getState(), source.id);
      location = (0, _location.createLocation)({ ...location,
        sourceActor
      });
    }

    const lastSelectedLocation = (0, _index2.getSelectedLocation)(getState());
    const {
      shouldSelectOriginalLocation,
      newLocation
    ...
    if ((0, _index2.getSelectedLocation)(getState()) != lastSelectedLocation) {
      return;
    ...
    if (!(0, _index2.tabExists)(getState(), source.id)) {
      dispatch((0, _tabs.addTab)(source, sourceActor));
    }

    dispatch(setSelectedLocation(location, shouldSelectOriginalLocation, highlight));
    ...
    if ((0, _index2.getSelectedLocation)(getState()) != location) {
      return;
    }
    ...
    const loadedSource = (0, _index2.getSource)(getState(), source.id);
    ...
    const sourceTextContent = (0, _index2.getSourceTextContent)(getState(), location);

    if (keepContext && _prefs.prefs.autoPrettyPrint && !(0, _index2.getPrettySource)(getState(), loadedSource.id) && (0, _index2.canPrettyPrintSource)(getState(), location) && (0, _source.isMinified)(source, sourceTextContent)) {
      await dispatch((0, _prettyPrint.prettyPrintAndSelectSource)(loadedSource));
      dispatch((0, _tabs.closeTab)(loadedSource));
    }

    await dispatch((0, _symbols.setSymbols)(location)); // Stop the async work if we started selecting another location
    ...
    dispatch((0, _index.setInScopeLines)()); // When we select a generated source which has a sourcemap,
    // asynchronously fetch the related original location in order to display
    // the mapped location in the editor's footer.

    if (!location.source.isOriginal && (0, _index2.isSourceActorWithSourceMap)(getState(), sourceActor.id)) {
...
function selectSpecificLocationOrSameUrl(location) {
  return async ({
    ...
    if (!(0, _index2.hasSource)(getState(), location.source.id)) {
      ...
      const source = (0, _index2.getSourceByURL)(getState(), location.source.url);
      ...
      const sourceActor = (0, _index2.getFirstSourceActorForGeneratedSource)(getState(), location.source.id);
      location = (0, _location.createLocation)({ ...location,
        source,
        sourceActor
      });
    } else if (!(0, _index2.hasSourceActor)(getState(), location.sourceActor.id)) {
...
function jumpToMappedSelectedLocation() {
  return async function ({
    ...
    const location = (0, _index2.getSelectedLocation)(getState());
