...
function initialSourceBlackBoxState(state) {
  return {
    ...
    blackboxedSet: (state === null || state === void 0 ? void 0 : state.blackboxedRanges) ? new Set(Object.keys(state.blackboxedRanges)) : new Set(),
  ...
  switch (action.type) {
    ...
    case "BLACKBOX_SOURCE_RANGES":
      {
        ...
        if (!currentBlackboxedRanges[source.url]) {
          ...
          currentBlackboxedRanges[source.url] = [...state.blackboxedRanges[source.url]];
        } // Add new blackboxed lines in acsending order


        for (const newRange of ranges) {
          const index = currentBlackboxedRanges[source.url].findIndex(range => range.end.line <= newRange.start.line && range.end.column <= newRange.start.column);
    ...
    case "UNBLACKBOX_SOURCE_RANGES":
      {
        ...
        for (const newRange of ranges) {
          const index = currentBlackboxedRanges[source.url].findIndex(range => range.start.line === newRange.start.line && range.end.line === newRange.end.line);
    ...
    case "ADD_SOURCEMAP_IGNORE_LIST_SOURCES":
      if (action.status == "done") {
        return { ...state,
          sourceMapIgnoreListUrls: [...state.sourceMapIgnoreListUrls, ...action.value]
        };
      }
