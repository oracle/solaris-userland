...
exports._mergeLexicalScopesBindings = _mergeLexicalScopesBindings;
exports.getScopeItemPath = getScopeItemPath;
exports.getScopesItemsForSelectedFrame = getScopesItemsForSelectedFrame;
...
} = _index.objectInspector; // The heading that should be displayed for the scope
...
function _getScopeTitle(type, scope) {
  ...
  if (type === "function" && scope.function) {
    return scope.function.displayName ? (0, _index2.simplifyDisplayName)(scope.function.displayName) : L10N.getStr("anonymousFunction");
  }
...
function _getScopeItem(scope, selectedFrame, frameScopes, why, scopeIndex) {
  ...
  if (type === "function" || type === "block") {
    ...
    let vars = _getBindingVariables(bindings, key); // show exception, return, and this variables in innermost scope
...
function _mergeLexicalScopesBindings(scope, parentScope, item, parentItem) {
...
function getScopeItemPath(item) {
...
function getScopesItemsForSelectedFrame(why, selectedFrame, frameScopes) {
  ...
  while (currentScope) {
    let currentScopeItem = _getScopeItem(currentScope, selectedFrame, frameScopes, why, currentScopeIndex);

    if (currentScopeItem) {
      const mergedItem = prevScope && prevScopeItem ? _mergeLexicalScopesBindings(prevScope, currentScope, prevScopeItem, currentScopeItem) : null;
