 ...
 */
async function attachTarget(targetFront, targets, options) {
  try {
...
async function attachTargets(targetLists, args) {
  ...
  targetLists = targetLists.filter(target => !!target); // First remove from the known list, targets which are no longer in the new list
...
async function listWorkerTargets(args) {
  ...
  if ((0, _events.attachAllTargets)(currentTarget)) {
    workers = await devToolsClient.mainRoot.listAllWorkerTargets(); // subprocess workers are ignored because they take several seconds to
    ...
    if (currentTarget.url && _prefs.features.windowlessServiceWorkers) {
      ...
      await pauseMatchingServiceWorkers({
        devToolsClient,
        currentTarget
      });
    }
...
async function pauseMatchingServiceWorkers({
  devToolsClient,
  currentTarget
}) {
  ...
  const origin = new URL(currentTarget.url).origin; // Still call `RootFront.listProcesses` instead of using the TargetList
  ...
  } catch (e) {// currentTarget.url might not be a full URL, and old servers without
...
async function listProcessTargets(args) {
  const {
    targetList
  } = args; // First note that the TargetList will only fetch processes following the same
