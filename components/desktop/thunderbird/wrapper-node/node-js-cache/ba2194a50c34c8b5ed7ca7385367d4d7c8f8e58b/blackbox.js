  ...
  return async thunkArgs => {
    ...
    if (shouldBlackBox) {
      ...
      if (!ranges.length) {
        ...
        const currentRanges = (0, _selectors.getBlackBoxRanges)(getState())[source.url] || [];
      ...
      if (!ranges.length) {
        ...
        const blackboxRanges = (0, _selectors.getBlackBoxRanges)(getState());
...
async function toggleBreakpointsInRangesForBlackboxedSource({
  ...
  for (const range of ranges) {
    const breakpoints = (0, _selectors.getBreakpointsForSource)(getState(), source.id, range);
    await dispatch((0, _breakpoints.toggleBreakpoints)(cx, shouldDisable, breakpoints));
  }
}
