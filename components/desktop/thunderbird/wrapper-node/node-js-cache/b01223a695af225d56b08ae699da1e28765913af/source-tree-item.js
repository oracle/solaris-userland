 ...
 */
function showSourceTreeItemContextMenu(event, item, depth, setExpanded, itemName) {
  return async ({
    dispatch,
    getState
  }) => {
    ...
    const isOverridden = (0, _index.isSourceOverridden)(state, item.source);
    ...
    if (item.type != "source") {
      ...
      if (projectRoot == item.uniquePath) {
        ...
        menuOptions.push({
          ...
          disabled: false,
          click: () => dispatch((0, _sourcesTree.setProjectDirectoryRoot)(item.uniquePath, itemName))
        });
      }
...
async function handleLocalOverride(dispatch, source, isOverridden) {
  if (!isOverridden) {
    ...
    if (localPath) {
      dispatch((0, _index2.setOverrideSource)(source, localPath));
    }
  } else {
    dispatch((0, _index2.removeOverrideSource)(source));
  }
}
