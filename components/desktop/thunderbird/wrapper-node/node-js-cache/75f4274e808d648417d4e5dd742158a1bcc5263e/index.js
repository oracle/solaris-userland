    ...
    for (const name of Object.keys(item.bindings)) {
      for (const ref of item.bindings[name].refs) {
        ...
        for (const loc of locs) {
          loc.start = (0, _location.sourceMapToDebuggerLocation)(state, loc.start);
          loc.end = (0, _location.sourceMapToDebuggerLocation)(state, loc.end);
        }
      }
    }
  }
...
async function buildMappedScopes(source, content, frame, generatedScopes, thunkArgs) {
  const {
    getState,
    parserWorker
  } = thunkArgs;
  ...
  const originalRanges = await (0, _rangeMetadata.loadRangeMetadata)(frame.location, originalAstScopes, thunkArgs);
  ...
  if (generatedScopes) {
    generatedAstBindings = (0, _buildGeneratedBindingList.buildGeneratedBindingList)(generatedScopes, generatedAstScopes, frame.this);
  } else {
  ...
  const {
    ...
    expressionLookup
  } = await mapOriginalBindingsToGenerated(source, content, originalRanges, originalAstScopes, generatedAstBindings, thunkArgs);
  const globalLexicalScope = generatedScopes ? getGlobalFromScope(generatedScopes) : generateGlobalFromAst(generatedAstScopes);
...
async function mapOriginalBindingsToGenerated(source, content, originalRanges, originalAstScopes, generatedAstBindings, thunkArgs) {
  const expressionLookup = {};
  const mappedOriginalScopes = [];
  const cachedSourceMaps = batchScopeMappings(originalAstScopes, source, thunkArgs); // Override sourceMapLoader attribute with the special cached SourceMapLoader instance
  // in order to make it used by all functions used in this method.

  thunkArgs = { ...thunkArgs,
    sourceMapLoader: cachedSourceMaps
  };
  ...
  for (const item of originalAstScopes) {
    ...
    for (const name of Object.keys(item.bindings)) {
      ...
      const result = await findGeneratedBinding(source, content, name, binding, originalRanges, generatedAstBindings, thunkArgs);
  ...
  for (const item of originalAstScopes) {
    for (const name of Object.keys(item.bindings)) {
      for (const ref of item.bindings[name].refs) {
        ...
        for (const loc of locs) {
          precalculatedRanges.set(buildLocationKey(loc.start), sourceMapLoader.getGeneratedRanges((0, _location.debuggerToSourceMapLocation)(loc.start)));
          precalculatedLocations.set(buildLocationKey(loc.start), sourceMapLoader.getGeneratedLocation((0, _location.debuggerToSourceMapLocation)(loc.start)));
          precalculatedLocations.set(buildLocationKey(loc.end), sourceMapLoader.getGeneratedLocation((0, _location.debuggerToSourceMapLocation)(loc.end)));
        }
      }
    }
  }
  ...
  return {
    async getGeneratedRanges(pos) {
      ...
      if (!precalculatedRanges.has(key)) {
        ...
        return sourceMapLoader.getGeneratedRanges((0, _location.debuggerToSourceMapLocation)(pos));
      }
    ...
    async getGeneratedLocation(pos) {
      ...
      if (!precalculatedLocations.has(key)) {
        (0, _log.log)("Bad precalculated mapping");
        return sourceMapLoader.getGeneratedLocation((0, _location.debuggerToSourceMapLocation)(pos));
      }
...
async function findGeneratedBinding(source, content, name, originalBinding, originalRanges, generatedAstBindings, thunkArgs) {
  ...
  const loadApplicableBindings = async (pos, locationType) => {
    let applicableBindings = await (0, _getApplicableBindingsForOriginalPosition.getApplicableBindingsForOriginalPosition)(generatedAstBindings, source, pos, originalBinding.type, locationType, thunkArgs);
    ...
    if (locationType !== "ref" && !(await (0, _getApplicableBindingsForOriginalPosition.originalRangeStartsInside)(pos, thunkArgs))) {
      applicableBindings = [];
    }
