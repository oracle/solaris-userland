...
function initialSourcesState(state) {
  ...
  return {
     ...
     */
    mutableOverrideSources: (state === null || state === void 0 ? void 0 : state.mutableOverrideSources) || new Map(),
...
function update(state = initialSourcesState(), action) {
  switch (action.type) {
    ...
    case "SET_ORIGINAL_BREAKABLE_LINES":
      {
        state.mutableOriginalBreakableLines.set(action.sourceId, action.breakableLines);
        return { ...state
        };
      }
...
function removeSourcesAndActors(state, action) {
  ...
  for (const removedSource of action.sources) {
    ...
    if (((_newState$selectedLoc = newState.selectedLocation) === null || _newState$selectedLoc === void 0 ? void 0 : _newState$selectedLoc.source) == removedSource) {
      newState.selectedLocation = null;
    }
  }

  for (const removedActor of action.actors) {
    var _newState$selectedLoc2;
    ...
    if (((_newState$selectedLoc2 = newState.selectedLocation) === null || _newState$selectedLoc2 === void 0 ? void 0 : _newState$selectedLoc2.sourceActor) == removedActor) {
      newState.selectedLocation = null;
    }
  }
