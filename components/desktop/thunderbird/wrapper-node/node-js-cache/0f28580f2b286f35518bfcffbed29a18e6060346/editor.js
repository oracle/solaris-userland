...
const blackBoxMenuItem = (cx, selectedSource, blackboxedRanges, editorActions, isSourceOnIgnoreList) => {
...
const blackBoxLineMenuItem = (cx, selectedSource, editorActions, editor, blackboxedRanges, isSourceOnIgnoreList, // the clickedLine is passed when the context menu
...
const blackBoxLinesMenuItem = (cx, selectedSource, editorActions, editor, blackboxedRanges, isSourceOnIgnoreList) => {
  const {
    codeMirror
  } = editor;
  const from = codeMirror.getCursor("from");
...
function editorMenuItems({
  ...
  items.push(jumpToMappedLocationItem(cx, selectedSource, location, hasMappedLocation, editorActions), continueToHereItem(cx, location, isPaused, editorActions), {
    ...
    type: "separator"
  }, blackBoxMenuItem(cx, selectedSource, blackboxedRanges, editorActions, isSourceOnIgnoreList));
  ...
  if (!theWholeSourceIsBlackBoxed) {
    ...
    items.push(blackBoxSourceLinesMenuItem(cx, selectedSource, editorActions, editor, blackboxedRanges, isSourceOnIgnoreList));
  }
