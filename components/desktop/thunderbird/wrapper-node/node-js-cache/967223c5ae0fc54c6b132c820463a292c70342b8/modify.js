...
async function clientSetBreakpoint(client, cx, {
  ...
  const breakpointServerLocation = (0, _breakpoint.makeBreakpointServerLocation)(getState(), breakpoint.generatedLocation);
  const shouldMapBreakpointExpressions = (0, _selectors.isMapScopesEnabled)(getState()) && breakpoint.location.source.isOriginal && (breakpoint.options.logValue || breakpoint.options.condition);
...
function clientRemoveBreakpoint(client, state, generatedLocation) {
  const breakpointServerLocation = (0, _breakpoint.makeBreakpointServerLocation)(state, generatedLocation);
...
function enableBreakpoint(cx, initialBreakpoint) {
  return thunkArgs => {
    ...
    const breakpoint = (0, _selectors.getBreakpoint)(state, initialBreakpoint.location);
    const blackboxedRanges = (0, _selectors.getBlackBoxRanges)(state);
    const isSourceOnIgnoreList = (0, _selectors.isSourceMapIgnoreListEnabled)(state) && (0, _selectors.isSourceOnSourceMapIgnoreList)(state, breakpoint.location.source);
...
function addBreakpoint(cx, initialLocation, options = {}, disabled, shouldCancel = () => false) {
  return async thunkArgs => {
    ...
    const position = initialLocation.column ? (0, _selectors.getBreakpointPositionsForLocation)(getState(), initialLocation) : (0, _selectors.getFirstBreakpointPosition)(getState(), initialLocation); // No position is found if the `initialLocation` is on a non-breakable line or
