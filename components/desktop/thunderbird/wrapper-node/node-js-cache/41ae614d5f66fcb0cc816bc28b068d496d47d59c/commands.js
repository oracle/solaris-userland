...
function createObjectFront(grip, frame) {
  ...
  const threadFront = (frame === null || frame === void 0 ? void 0 : frame.thread) ? lookupThreadFront(frame.thread) : currentThreadFront();
...
async function startTracing(logMethod) {
  const targets = commands.targetCommand.getAllTargets(commands.targetCommand.ALL_TYPES);
  await Promise.all(targets.map(async targetFront => {
...
function hasBreakpoint(location) {
  return !!breakpoints[(0, _breakpoint.makeBreakpointServerLocationId)(location)];
}
...
async function setBreakpoint(location, options) {
  const breakpoint = breakpoints[(0, _breakpoint.makeBreakpointServerLocationId)(location)];

  if (breakpoint && JSON.stringify(breakpoint.options) == JSON.stringify(options)) {
    return null;
  }

  breakpoints[(0, _breakpoint.makeBreakpointServerLocationId)(location)] = {
    location,
    options
...
async function removeBreakpoint(location) {
  delete breakpoints[(0, _breakpoint.makeBreakpointServerLocationId)(location)];
  const hasWatcherSupport = commands.targetCommand.hasTargetWatcherSupport();
